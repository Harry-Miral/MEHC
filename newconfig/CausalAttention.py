"""
CausalAttention.py - Standard Causal Attention with FlexAttention
Implements standard causal mask with interface fully compatible with SPAttention
"""
...
class SPAttentionCache:
    """Standard causal attention mask cache manager (compatible with SPAttention interface)"""
    ...
    def _create_balanced_mask(self, seq_len: int) -> torch.Tensor:
        """Create standard causal mask (compatible with SPAttention interface)"""
        def mask_fn(b, h, i, j):
            # Standard causal mask: can only see current position and previous positions
            return j <= i
