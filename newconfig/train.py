            ...
            with grad_sync_context():
                ...
                if self.model.config.block_type == BlockType.moe:
                    ...
                    if self.model.config.moe_zloss_weight:
                        # batched_router_zloss 返回每层的 z-loss，需要求和
                        moe_z_loss = batched_router_zloss(self.moe_args).sum() / len(micro_batches)
                    ...
                    # 将损失添加到总损失中
                    if self.model.config.moe_loss_weight and lb_loss is not None:
                        loss += lb_loss
                        lb_batch_loss += lb_loss.detach()

                    if self.model.config.moe_zloss_weight and moe_z_loss is not None:
                        loss += moe_z_loss
