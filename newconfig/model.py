...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if config.attention_layer_norm:
            ...
            # Change normalization to operate on per-head dimension
            self.k_norm = LayerNormBase.build(
                config,
                size=head_dim,  # Now normalizing per head dimension instead of across all heads
                elementwise_affine=config.attention_layer_norm_with_affine,
            )
            self.q_norm = LayerNormBase.build(
                config, 
                size=head_dim,  # Now normalizing per head dimension
                elementwise_affine=config.attention_layer_norm_with_affine,
            )
        ...
        if config.flash_attention:
            ...
            N_HEADS = 16  # It can be changed to 8/16
            MODE = "short"  # You can change it to "long" or "short".

            # Create and precompile cache
            self.sp_cache = create_sp_attention_cache(n_heads=N_HEADS, mode=MODE)
            self.sp_cache.precompile(verbose=True)
    ...
    def _scaled_dot_product_attention(
        ...
        if self.sparse_attention is not None and attn_mask is None:
            r = self.sparse_attention(
                q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), self.sp_cache
            )
            return r.transpose(1, 2)
        else:
            print("error:sparse_attention is not support!")
    ...
    def attention(
        ...
        # After splitting into heads, normalize each head.
        if self.q_norm is not None and self.k_norm is not None:
            # Here, a new per-head normalization layer needs to be created to replace the original self.q_norm and self.k_norm.
            head_dim = C // self.config.n_heads
            # [B, nh, T, hs] -> [B*nh, T, hs]
            q_reshaped = q.contiguous().view(B * self.config.n_heads, T, head_dim)
            q_normed = self.q_norm(q_reshaped).to(dtype=dtype)
            q = q_normed.view(B, self.config.n_heads, T, head_dim)

            # [B, n_kv_h, T, hs] -> [B*n_kv_h, T, hs]
            k_reshaped = k.contiguous().view(B * self.config.effective_n_kv_heads, T, head_dim)
            k_normed = self.k_norm(k_reshaped).to(dtype=dtype)
            k = k_normed.view(B, self.config.effective_n_kv_heads, T, head_dim)
