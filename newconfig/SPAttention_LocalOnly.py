"""
SPAttention_Config2.py - Ablation Experiment A (Local-Only Attention)
Remove global connection bands, all heads become functionally identical local attention heads
"""
...
class SPAttentionCache:
    """Sparse attention mask cache manager - Config 2: Local-only attention"""
    ...
    def __init__(self, n_heads: int, length_mode: LengthMode, device: str = 'cuda'):
        ...
        print(f"Initialize SPAttention cache [Config 2-Local Only]: {n_heads} heads, {length_mode.value} length range")

    def _create_balanced_mask(self, seq_len: int) -> torch.Tensor:
        """Create local-only attention mask (strict ablation experiment A)"""
        # Keep original balanced allocation logic, completely unchanged
        ...
        def mask_fn(b, h, i, j):
            ...
            # Keep original dynamic head_width calculation logic
            # First extra_tokens heads have bandwidth base_width + 1, rest have base_width
            ...
            # Local band - keep original logic unchanged
            local = (i - j) <= head_width

            # Strict ablation: only remove global connection band calculation and usage
            # Original version's global band calculation logic (commented out, not executed):
            ...
            # in_global_range = distance > head_width
            # in_global = in_global_start & in_global_end & in_global_range

            # Core modification of strict ablation A: only remove | in_global
            # Original version: return causal & (local | in_global)
            # Config 2: return causal & local
            return causal & local
    ...
    def precompile(self, verbose: bool = True) -> None:
        ...
        print(f"\nPrecompilation completed [Config 2-Local Only]:")
